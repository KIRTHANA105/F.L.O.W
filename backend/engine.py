...
def simulate(workflow, dataset_name=None):
    ...
    id_key = "invoice_id" if dataset_name == "invoices" else "order_id"
    results = []
    evaluations = 0
    ...
    noun = "invoices" if dataset_name == "invoices" else "orders"
    return {
        "dataset": dataset_name,
        "id_label": "Invoice ID" if dataset_name == "invoices" else "Order ID",
        "total": len(records),
        "matched": match_count,
...
def count_affected(conflict):
    ...
    id_key = "invoice_id" if dataset == "invoices" else "order_id"
    ids = []
    for rec in records:
